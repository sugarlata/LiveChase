...
sheoaks,Sheoaks,-37.91,144.13,236.7
...
eildon-fire-tower,Eildon Fire Tower,-37.21,145.84,637"""
    ...
    for j in range(0, len(obs_list)):
        ...
        try:
            os.chdir(root_folder + date_path + "\Observations")
        except WindowsError:
            os.mkdir(root_folder + date_path + "\Observations")
        ...
        local_time = str(arrow.now().format('YYYY-MM-DD')) + " " + str(arrow.get(obs_list[j].time, 'HH:mmA').format('HH:mm'))
        ts = arrow.get(local_time, 'YYYY-MM-DD HH:mm', tzinfo='Australia/Melbourne').timestamp
